...
export default class HomeMaster extends Component {
    ...
    if (isSuperAdmin)
      return (
        <ul className="dropdown-menu" role="menu">
          ...
          <li>
            <Link to="settings/rab_saas_site">Settings</Link>
          </li>
          <li>
  ...
  createUserView() {
    var userPhoto = this.state.user.photoURL
      ? this.state.user.photoURL
      : "http://www.gravatar.com/avatar/" +
        md5(this.state.user.email + "") +
        "?s=512";
    return (
      <li className="dropdown userDropdown">
        <a href="#" className="dropdown-toggle" data-toggle="dropdown">
          <img
            alt=""
            className="img-circle img-responsive fireadmin-user_image"
            src={userPhoto}
          />
        </a>
              ...
              <ul className="nav navbar-nav navbar-right">
                <ConditionalDisplay condition={this.state.user.displayName}>
                  <li style={{ "padding-top": "10px" }}>
                    <Link
                      to="/account"
                      style={{ "padding-left": "73px", "padding-right": "2px" }}
                    >
